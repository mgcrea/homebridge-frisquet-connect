...
export const getFrisquetConnectAccessorySetup = (
  accessory: PlatformAccessory,
): FrisquetConnectAccessorySetup => {
...
export const setupAccessoryTemperatureHistoryService = (
  ...
  // @ts-expect-error fake-gato types are incomplete
  // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
  const historyService = new HistoryService("weather", accessory, {
    storage: "fs",
  });
      ...
      .then((value) => {
        if (typeof value === "number" && value) {
          ...
          controller.log.debug(
            `Accessory named="${accessory.displayName}" has added entry="${JSON.stringify(entry)}"`,
          );
        }
      })
      .catch((err: unknown) => {
        controller.log.warn(`Failed to get temperature for history: ${String(err)}`);
      });
