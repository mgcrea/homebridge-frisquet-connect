...
import got, { Got, type AfterResponseHook, type BeforeRequestHook } from "got";
import type { Logging } from "homebridge";
import { DEFAULT_APP_ID, DEFAULT_USER_AGENT, HOMEBRIDGE_FRISQUET_CONNECT_PASSWORD } from "./config/env";
...
const clientFactory = (log: Logging, config: FrisquetConnectPlatformConfig): Client => {
  ...
  const password = HOMEBRIDGE_FRISQUET_CONNECT_PASSWORD
    ? decode(HOMEBRIDGE_FRISQUET_CONNECT_PASSWORD)
    : configPassword;
    ...
    hooks: {
      beforeRequest: [
        ((options) => {
          ...
          debug(`About to request url="${String(url)}" with method="${method}"`);
